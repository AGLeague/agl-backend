...
function getSearchParamAsNumber(url: URL, param: string, defaultValue: number): number {
...
router.get("", async function(req, res) {
	...
	let page = getSearchParamAsNumber(url, "page", 1)
	// If we are aiming for a binder appearance, 3x4 as a default?
	let size = getSearchParamAsNumber(url, "size", 12)
	...
	let statsSheet = await sheets.spreadsheets.get({
	...
	let sheetName = "Cumulative Record";

	let recordSheet = statsSheet.data.sheets.find(sheet => sheet.properties && sheet.properties.title === sheetName)

	let rowCount = (recordSheet?.properties?.gridProperties?.rowCount) ?? -1;

	// The First row is a header, not a player.
	let playerCount = rowCount - 1
	...
	let startRow = ((page - 1) * size) + 2
	let endRow = startRow + size - 1

	let range = sheetName + "!E" + startRow + ":I" + endRow

	var results = await sheets.spreadsheets.values.get({
		spreadsheetId: STATS_SHEET_ID,
		range: range
	})

	let response: PaginatedResponse<Players> = {
